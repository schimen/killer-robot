...
void motor_init(
    ...
    motor_a->thread = k_thread_create(
        &motor_threads[0], stacks[0], STACK_SIZE,
        motor_speed_thread_callback, motor_a, NULL, NULL,
    ...
    motor_b->thread = k_thread_create(
        &motor_threads[1], stacks[1], STACK_SIZE,
        motor_speed_thread_callback, motor_b, NULL, NULL,
        THREAD_PRIORITY, 0, K_NO_WAIT
    );
    ...
    motor_w->thread = k_thread_create(
        &motor_threads[2], stacks[2], STACK_SIZE,
        motor_speed_thread_callback, motor_w, NULL, NULL,
        THREAD_PRIORITY, 0, K_NO_WAIT
    );
}
...
int set_weapon_speed(const struct pwm_dt_spec *weapon_pin, const struct pwm_dt_spec *unused, uint8_t speed_u) {
    ARG_UNUSED(unused);
    int32_t speed = convert_speed(speed_u);
...
void motor_speed_thread_callback(void *p1, void *p2, void *p3) {
    ...
    int (*set_speed_func)(
        const struct pwm_dt_spec*, const struct pwm_dt_spec*, uint8_t
